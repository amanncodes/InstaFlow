...
from app.integrations.backend import emit_event
from ..services.avatar_service import avatar_exists, fetch_and_save_avatar
import time
...
class InstagramBot(BaseBot):
    ...
    def _find_password_field(self, username_input, wait_for_field=False):
        ...
        if wait_for_field:
            ...
            if use_tab and username_input and self.driver:
    ...
    def _finalize_login(self):
        ...
        if success:
            ...
            self.save_cookies()
            first_login = self.account.last_login is None
            ...
            emit_event(
                ...
                payload={
                    ...
                    "cookiesCount": len(self.driver.get_cookies()) if self.driver else 0,
                    "description": "Login confirmed and cookies saved."
                }
            )
            if first_login and not avatar_exists(self.account):
                try:
                    paths = list(fetch_and_save_avatar(self.account))
                    emit_event(
                        account_id=self.account.username,
                        ...
                        payload={
                            "action": "avatar_fetch",
                            "username": self.account.username,
                            "description": "Fetched profile avatar immediately after first login.",
                    ...
                    emit_event(
                        ...
                        payload={
                            "action": "avatar_fetch",
                            "username": self.account.username,
                            "description": "Avatar fetch failed after first login.",
                            "error": str(e)
                        }
                    )
            self.close_browser()
